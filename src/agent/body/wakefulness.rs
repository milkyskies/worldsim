...
pub fn tick_wakefulness(
    ...
    for (active, mut physical, mut consciousness, phenotype) in query.iter_mut() {
        ...
        if is_sleeping {
            ...
            physical.wakefulness =
                (physical.wakefulness + SLEEP_RESTORE_RATE * efficiency * dt).min(1.0);
        } else {
            let circadian_multiplier =
                1.0 + CIRCADIAN_NIGHT_BOOST * (CIRCADIAN_LIGHT_CEILING - light.0).max(0.0);
            physical.wakefulness =
                (physical.wakefulness - ADENOSINE_RATE * circadian_multiplier * dt).max(0.0);
        }
        ...
        let deficit = 1.0 - physical.wakefulness;
